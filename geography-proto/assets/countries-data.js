// Generated database of countries and flight geography presets
window.COUNTRIES_DATA = {
  "BD": {
    "code": "BD",
    "nameRu": "Бангладеш",
    "nameEn": "Bangladesh",
    "flag": "🇧🇩"
  },
  "BE": {
    "code": "BE",
    "nameRu": "Бельгия",
    "nameEn": "Belgium",
    "flag": "🇧🇪"
  },
  "BF": {
    "code": "BF",
    "nameRu": "Burkina Faso",
    "nameEn": "Burkina Faso",
    "flag": "🇧🇫"
  },
  "BG": {
    "code": "BG",
    "nameRu": "Болгария",
    "nameEn": "Bulgaria",
    "flag": "🇧🇬"
  },
  "BA": {
    "code": "BA",
    "nameRu": "Босния и Герцеговина",
    "nameEn": "Bosnia and Herz.",
    "flag": "🇧🇦"
  },
  "BN": {
    "code": "BN",
    "nameRu": "Brunei",
    "nameEn": "Brunei",
    "flag": "🇧🇳"
  },
  "BO": {
    "code": "BO",
    "nameRu": "Bolivia",
    "nameEn": "Bolivia",
    "flag": "🇧🇴"
  },
  "JP": {
    "code": "JP",
    "nameRu": "Япония",
    "nameEn": "Japan",
    "flag": "🇯🇵"
  },
  "BI": {
    "code": "BI",
    "nameRu": "Burundi",
    "nameEn": "Burundi",
    "flag": "🇧🇮"
  },
  "BJ": {
    "code": "BJ",
    "nameRu": "Benin",
    "nameEn": "Benin",
    "flag": "🇧🇯"
  },
  "BT": {
    "code": "BT",
    "nameRu": "Bhutan",
    "nameEn": "Bhutan",
    "flag": "🇧🇹"
  },
  "JM": {
    "code": "JM",
    "nameRu": "Jamaica",
    "nameEn": "Jamaica",
    "flag": "🇯🇲"
  },
  "BW": {
    "code": "BW",
    "nameRu": "Botswana",
    "nameEn": "Botswana",
    "flag": "🇧🇼"
  },
  "BR": {
    "code": "BR",
    "nameRu": "Бразилия",
    "nameEn": "Brazil",
    "flag": "🇧🇷"
  },
  "BS": {
    "code": "BS",
    "nameRu": "Bahamas",
    "nameEn": "Bahamas",
    "flag": "🇧🇸"
  },
  "BY": {
    "code": "BY",
    "nameRu": "Беларусь",
    "nameEn": "Belarus",
    "flag": "🇧🇾"
  },
  "BZ": {
    "code": "BZ",
    "nameRu": "Belize",
    "nameEn": "Belize",
    "flag": "🇧🇿"
  },
  "RU": {
    "code": "RU",
    "nameRu": "Россия",
    "nameEn": "Russia",
    "flag": "🇷🇺"
  },
  "RW": {
    "code": "RW",
    "nameRu": "Rwanda",
    "nameEn": "Rwanda",
    "flag": "🇷🇼"
  },
  "RS": {
    "code": "RS",
    "nameRu": "Сербия",
    "nameEn": "Serbia",
    "flag": "🇷🇸"
  },
  "LT": {
    "code": "LT",
    "nameRu": "Литва",
    "nameEn": "Lithuania",
    "flag": "🇱🇹"
  },
  "LU": {
    "code": "LU",
    "nameRu": "Люксембург",
    "nameEn": "Luxembourg",
    "flag": "🇱🇺"
  },
  "LR": {
    "code": "LR",
    "nameRu": "Liberia",
    "nameEn": "Liberia",
    "flag": "🇱🇷"
  },
  "RO": {
    "code": "RO",
    "nameRu": "Румыния",
    "nameEn": "Romania",
    "flag": "🇷🇴"
  },
  "GW": {
    "code": "GW",
    "nameRu": "Guinea-Bissau",
    "nameEn": "Guinea-Bissau",
    "flag": "🇬🇼"
  },
  "GT": {
    "code": "GT",
    "nameRu": "Guatemala",
    "nameEn": "Guatemala",
    "flag": "🇬🇹"
  },
  "GR": {
    "code": "GR",
    "nameRu": "Греция",
    "nameEn": "Greece",
    "flag": "🇬🇷"
  },
  "GQ": {
    "code": "GQ",
    "nameRu": "Eq. Guinea",
    "nameEn": "Eq. Guinea",
    "flag": "🇬🇶"
  },
  "GY": {
    "code": "GY",
    "nameRu": "Guyana",
    "nameEn": "Guyana",
    "flag": "🇬🇾"
  },
  "GE": {
    "code": "GE",
    "nameRu": "Грузия",
    "nameEn": "Georgia",
    "flag": "🇬🇪"
  },
  "GB": {
    "code": "GB",
    "nameRu": "Великобритания",
    "nameEn": "United Kingdom",
    "flag": "🇬🇧"
  },
  "GA": {
    "code": "GA",
    "nameRu": "Gabon",
    "nameEn": "Gabon",
    "flag": "🇬🇦"
  },
  "GN": {
    "code": "GN",
    "nameRu": "Guinea",
    "nameEn": "Guinea",
    "flag": "🇬🇳"
  },
  "GM": {
    "code": "GM",
    "nameRu": "Gambia",
    "nameEn": "Gambia",
    "flag": "🇬🇲"
  },
  "GL": {
    "code": "GL",
    "nameRu": "Greenland",
    "nameEn": "Greenland",
    "flag": "🇬🇱"
  },
  "KW": {
    "code": "KW",
    "nameRu": "Кувейт",
    "nameEn": "Kuwait",
    "flag": "🇰🇼"
  },
  "GH": {
    "code": "GH",
    "nameRu": "Гана",
    "nameEn": "Ghana",
    "flag": "🇬🇭"
  },
  "OM": {
    "code": "OM",
    "nameRu": "Оман",
    "nameEn": "Oman",
    "flag": "🇴🇲"
  },
  "_2": {
    "code": "_2",
    "nameRu": "Somaliland",
    "nameEn": "Somaliland",
    "flag": "🌐"
  },
  "_1": {
    "code": "_1",
    "nameRu": "Косово",
    "nameEn": "Kosovo",
    "flag": "🇽🇰"
  },
  "_0": {
    "code": "_0",
    "nameRu": "N. Cyprus",
    "nameEn": "N. Cyprus",
    "flag": "🌐"
  },
  "JO": {
    "code": "JO",
    "nameRu": "Иордания",
    "nameEn": "Jordan",
    "flag": "🇯🇴"
  },
  "HR": {
    "code": "HR",
    "nameRu": "Хорватия",
    "nameEn": "Croatia",
    "flag": "🇭🇷"
  },
  "HT": {
    "code": "HT",
    "nameRu": "Haiti",
    "nameEn": "Haiti",
    "flag": "🇭🇹"
  },
  "HU": {
    "code": "HU",
    "nameRu": "Венгрия",
    "nameEn": "Hungary",
    "flag": "🇭🇺"
  },
  "HN": {
    "code": "HN",
    "nameRu": "Honduras",
    "nameEn": "Honduras",
    "flag": "🇭🇳"
  },
  "PR": {
    "code": "PR",
    "nameRu": "Puerto Rico",
    "nameEn": "Puerto Rico",
    "flag": "🇵🇷"
  },
  "PS": {
    "code": "PS",
    "nameRu": "Palestine",
    "nameEn": "Palestine",
    "flag": "🇵🇸"
  },
  "PT": {
    "code": "PT",
    "nameRu": "Португалия",
    "nameEn": "Portugal",
    "flag": "🇵🇹"
  },
  "PY": {
    "code": "PY",
    "nameRu": "Paraguay",
    "nameEn": "Paraguay",
    "flag": "🇵🇾"
  },
  "PA": {
    "code": "PA",
    "nameRu": "Панама",
    "nameEn": "Panama",
    "flag": "🇵🇦"
  },
  "PG": {
    "code": "PG",
    "nameRu": "Папуа — Новая Гвинея",
    "nameEn": "Papua New Guinea",
    "flag": "🇵🇬"
  },
  "PE": {
    "code": "PE",
    "nameRu": "Перу",
    "nameEn": "Peru",
    "flag": "🇵🇪"
  },
  "PK": {
    "code": "PK",
    "nameRu": "Пакистан",
    "nameEn": "Pakistan",
    "flag": "🇵🇰"
  },
  "PH": {
    "code": "PH",
    "nameRu": "Филиппины",
    "nameEn": "Philippines",
    "flag": "🇵🇭"
  },
  "PL": {
    "code": "PL",
    "nameRu": "Польша",
    "nameEn": "Poland",
    "flag": "🇵🇱"
  },
  "ZM": {
    "code": "ZM",
    "nameRu": "Zambia",
    "nameEn": "Zambia",
    "flag": "🇿🇲"
  },
  "EH": {
    "code": "EH",
    "nameRu": "W. Sahara",
    "nameEn": "W. Sahara",
    "flag": "🇪🇭"
  },
  "EE": {
    "code": "EE",
    "nameRu": "Эстония",
    "nameEn": "Estonia",
    "flag": "🇪🇪"
  },
  "EG": {
    "code": "EG",
    "nameRu": "Египет",
    "nameEn": "Egypt",
    "flag": "🇪🇬"
  },
  "ZA": {
    "code": "ZA",
    "nameRu": "ЮАР",
    "nameEn": "South Africa",
    "flag": "🇿🇦"
  },
  "EC": {
    "code": "EC",
    "nameRu": "Эквадор",
    "nameEn": "Ecuador",
    "flag": "🇪🇨"
  },
  "AL": {
    "code": "AL",
    "nameRu": "Албания",
    "nameEn": "Albania",
    "flag": "🇦🇱"
  },
  "AO": {
    "code": "AO",
    "nameRu": "Angola",
    "nameEn": "Angola",
    "flag": "🇦🇴"
  },
  "KZ": {
    "code": "KZ",
    "nameRu": "Казахстан",
    "nameEn": "Kazakhstan",
    "flag": "🇰🇿"
  },
  "ET": {
    "code": "ET",
    "nameRu": "Эфиопия",
    "nameEn": "Ethiopia",
    "flag": "🇪🇹"
  },
  "ZW": {
    "code": "ZW",
    "nameRu": "Zimbabwe",
    "nameEn": "Zimbabwe",
    "flag": "🇿🇼"
  },
  "ES": {
    "code": "ES",
    "nameRu": "Испания",
    "nameEn": "Spain",
    "flag": "🇪🇸"
  },
  "ER": {
    "code": "ER",
    "nameRu": "Eritrea",
    "nameEn": "Eritrea",
    "flag": "🇪🇷"
  },
  "ME": {
    "code": "ME",
    "nameRu": "Черногория",
    "nameEn": "Montenegro",
    "flag": "🇲🇪"
  },
  "MD": {
    "code": "MD",
    "nameRu": "Молдова",
    "nameEn": "Moldova",
    "flag": "🇲🇩"
  },
  "MG": {
    "code": "MG",
    "nameRu": "Madagascar",
    "nameEn": "Madagascar",
    "flag": "🇲🇬"
  },
  "MA": {
    "code": "MA",
    "nameRu": "Марокко",
    "nameEn": "Morocco",
    "flag": "🇲🇦"
  },
  "UZ": {
    "code": "UZ",
    "nameRu": "Узбекистан",
    "nameEn": "Uzbekistan",
    "flag": "🇺🇿"
  },
  "MM": {
    "code": "MM",
    "nameRu": "Мьянма",
    "nameEn": "Myanmar",
    "flag": "🇲🇲"
  },
  "ML": {
    "code": "ML",
    "nameRu": "Mali",
    "nameEn": "Mali",
    "flag": "🇲🇱"
  },
  "MN": {
    "code": "MN",
    "nameRu": "Монголия",
    "nameEn": "Mongolia",
    "flag": "🇲🇳"
  },
  "MK": {
    "code": "MK",
    "nameRu": "Северная Македония",
    "nameEn": "Macedonia",
    "flag": "🇲🇰"
  },
  "MW": {
    "code": "MW",
    "nameRu": "Malawi",
    "nameEn": "Malawi",
    "flag": "🇲🇼"
  },
  "MR": {
    "code": "MR",
    "nameRu": "Mauritania",
    "nameEn": "Mauritania",
    "flag": "🇲🇷"
  },
  "UG": {
    "code": "UG",
    "nameRu": "Уганда",
    "nameEn": "Uganda",
    "flag": "🇺🇬"
  },
  "MY": {
    "code": "MY",
    "nameRu": "Малайзия",
    "nameEn": "Malaysia",
    "flag": "🇲🇾"
  },
  "MX": {
    "code": "MX",
    "nameRu": "Мексика",
    "nameEn": "Mexico",
    "flag": "🇲🇽"
  },
  "VU": {
    "code": "VU",
    "nameRu": "Vanuatu",
    "nameEn": "Vanuatu",
    "flag": "🇻🇺"
  },
  "FR": {
    "code": "FR",
    "nameRu": "Франция",
    "nameEn": "France",
    "flag": "🇫🇷"
  },
  "FI": {
    "code": "FI",
    "nameRu": "Финляндия",
    "nameEn": "Finland",
    "flag": "🇫🇮"
  },
  "FJ": {
    "code": "FJ",
    "nameRu": "Фиджи",
    "nameEn": "Fiji",
    "flag": "🇫🇯"
  },
  "FK": {
    "code": "FK",
    "nameRu": "Falkland Is.",
    "nameEn": "Falkland Is.",
    "flag": "🇫🇰"
  },
  "NI": {
    "code": "NI",
    "nameRu": "Nicaragua",
    "nameEn": "Nicaragua",
    "flag": "🇳🇮"
  },
  "NL": {
    "code": "NL",
    "nameRu": "Нидерланды",
    "nameEn": "Netherlands",
    "flag": "🇳🇱"
  },
  "NO": {
    "code": "NO",
    "nameRu": "Норвегия",
    "nameEn": "Norway",
    "flag": "🇳🇴"
  },
  "NA": {
    "code": "NA",
    "nameRu": "Namibia",
    "nameEn": "Namibia",
    "flag": "🇳🇦"
  },
  "NC": {
    "code": "NC",
    "nameRu": "New Caledonia",
    "nameEn": "New Caledonia",
    "flag": "🇳🇨"
  },
  "NE": {
    "code": "NE",
    "nameRu": "Niger",
    "nameEn": "Niger",
    "flag": "🇳🇪"
  },
  "NG": {
    "code": "NG",
    "nameRu": "Нигерия",
    "nameEn": "Nigeria",
    "flag": "🇳🇬"
  },
  "NZ": {
    "code": "NZ",
    "nameRu": "Новая Зеландия",
    "nameEn": "New Zealand",
    "flag": "🇳🇿"
  },
  "NP": {
    "code": "NP",
    "nameRu": "Непал",
    "nameEn": "Nepal",
    "flag": "🇳🇵"
  },
  "CI": {
    "code": "CI",
    "nameRu": "Кот-д’Ивуар",
    "nameEn": "Côte d'Ivoire",
    "flag": "🇨🇮"
  },
  "CH": {
    "code": "CH",
    "nameRu": "Швейцария",
    "nameEn": "Switzerland",
    "flag": "🇨🇭"
  },
  "CO": {
    "code": "CO",
    "nameRu": "Колумбия",
    "nameEn": "Colombia",
    "flag": "🇨🇴"
  },
  "CN": {
    "code": "CN",
    "nameRu": "Китай",
    "nameEn": "China",
    "flag": "🇨🇳"
  },
  "CM": {
    "code": "CM",
    "nameRu": "Cameroon",
    "nameEn": "Cameroon",
    "flag": "🇨🇲"
  },
  "CL": {
    "code": "CL",
    "nameRu": "Чили",
    "nameEn": "Chile",
    "flag": "🇨🇱"
  },
  "CA": {
    "code": "CA",
    "nameRu": "Канада",
    "nameEn": "Canada",
    "flag": "🇨🇦"
  },
  "CG": {
    "code": "CG",
    "nameRu": "Congo",
    "nameEn": "Congo",
    "flag": "🇨🇬"
  },
  "CF": {
    "code": "CF",
    "nameRu": "Central African Rep.",
    "nameEn": "Central African Rep.",
    "flag": "🇨🇫"
  },
  "CD": {
    "code": "CD",
    "nameRu": "Dem. Rep. Congo",
    "nameEn": "Dem. Rep. Congo",
    "flag": "🇨🇩"
  },
  "CZ": {
    "code": "CZ",
    "nameRu": "Чехия",
    "nameEn": "Czech Republic",
    "flag": "🇨🇿"
  },
  "CY": {
    "code": "CY",
    "nameRu": "Кипр",
    "nameEn": "Cyprus",
    "flag": "🇨🇾"
  },
  "CR": {
    "code": "CR",
    "nameRu": "Коста-Рика",
    "nameEn": "Costa Rica",
    "flag": "🇨🇷"
  },
  "CU": {
    "code": "CU",
    "nameRu": "Куба",
    "nameEn": "Cuba",
    "flag": "🇨🇺"
  },
  "SZ": {
    "code": "SZ",
    "nameRu": "Swaziland",
    "nameEn": "Swaziland",
    "flag": "🇸🇿"
  },
  "SY": {
    "code": "SY",
    "nameRu": "Сирия",
    "nameEn": "Syria",
    "flag": "🇸🇾"
  },
  "KG": {
    "code": "KG",
    "nameRu": "Кыргызстан",
    "nameEn": "Kyrgyzstan",
    "flag": "🇰🇬"
  },
  "KE": {
    "code": "KE",
    "nameRu": "Кения",
    "nameEn": "Kenya",
    "flag": "🇰🇪"
  },
  "SS": {
    "code": "SS",
    "nameRu": "S. Sudan",
    "nameEn": "S. Sudan",
    "flag": "🇸🇸"
  },
  "SR": {
    "code": "SR",
    "nameRu": "Suriname",
    "nameEn": "Suriname",
    "flag": "🇸🇷"
  },
  "KH": {
    "code": "KH",
    "nameRu": "Камбоджа",
    "nameEn": "Cambodia",
    "flag": "🇰🇭"
  },
  "SV": {
    "code": "SV",
    "nameRu": "El Salvador",
    "nameEn": "El Salvador",
    "flag": "🇸🇻"
  },
  "SK": {
    "code": "SK",
    "nameRu": "Словакия",
    "nameEn": "Slovakia",
    "flag": "🇸🇰"
  },
  "KR": {
    "code": "KR",
    "nameRu": "Южная Корея",
    "nameEn": "Korea",
    "flag": "🇰🇷"
  },
  "SI": {
    "code": "SI",
    "nameRu": "Словения",
    "nameEn": "Slovenia",
    "flag": "🇸🇮"
  },
  "KP": {
    "code": "KP",
    "nameRu": "Dem. Rep. Korea",
    "nameEn": "Dem. Rep. Korea",
    "flag": "🇰🇵"
  },
  "SO": {
    "code": "SO",
    "nameRu": "Somalia",
    "nameEn": "Somalia",
    "flag": "🇸🇴"
  },
  "SN": {
    "code": "SN",
    "nameRu": "Сенегал",
    "nameEn": "Senegal",
    "flag": "🇸🇳"
  },
  "SL": {
    "code": "SL",
    "nameRu": "Sierra Leone",
    "nameEn": "Sierra Leone",
    "flag": "🇸🇱"
  },
  "SB": {
    "code": "SB",
    "nameRu": "Solomon Is.",
    "nameEn": "Solomon Is.",
    "flag": "🇸🇧"
  },
  "SA": {
    "code": "SA",
    "nameRu": "Саудовская Аравия",
    "nameEn": "Saudi Arabia",
    "flag": "🇸🇦"
  },
  "SE": {
    "code": "SE",
    "nameRu": "Швеция",
    "nameEn": "Sweden",
    "flag": "🇸🇪"
  },
  "SD": {
    "code": "SD",
    "nameRu": "Sudan",
    "nameEn": "Sudan",
    "flag": "🇸🇩"
  },
  "DO": {
    "code": "DO",
    "nameRu": "Доминикана",
    "nameEn": "Dominican Rep.",
    "flag": "🇩🇴"
  },
  "DJ": {
    "code": "DJ",
    "nameRu": "Djibouti",
    "nameEn": "Djibouti",
    "flag": "🇩🇯"
  },
  "DK": {
    "code": "DK",
    "nameRu": "Дания",
    "nameEn": "Denmark",
    "flag": "🇩🇰"
  },
  "DE": {
    "code": "DE",
    "nameRu": "Германия",
    "nameEn": "Germany",
    "flag": "🇩🇪"
  },
  "YE": {
    "code": "YE",
    "nameRu": "Йемен",
    "nameEn": "Yemen",
    "flag": "🇾🇪"
  },
  "AT": {
    "code": "AT",
    "nameRu": "Австрия",
    "nameEn": "Austria",
    "flag": "🇦🇹"
  },
  "DZ": {
    "code": "DZ",
    "nameRu": "Алжир",
    "nameEn": "Algeria",
    "flag": "🇩🇿"
  },
  "US": {
    "code": "US",
    "nameRu": "США",
    "nameEn": "United States",
    "flag": "🇺🇸"
  },
  "LV": {
    "code": "LV",
    "nameRu": "Латвия",
    "nameEn": "Latvia",
    "flag": "🇱🇻"
  },
  "UY": {
    "code": "UY",
    "nameRu": "Uruguay",
    "nameEn": "Uruguay",
    "flag": "🇺🇾"
  },
  "LB": {
    "code": "LB",
    "nameRu": "Ливан",
    "nameEn": "Lebanon",
    "flag": "🇱🇧"
  },
  "LA": {
    "code": "LA",
    "nameRu": "Лаос",
    "nameEn": "Laos",
    "flag": "🇱🇦"
  },
  "TW": {
    "code": "TW",
    "nameRu": "Taiwan",
    "nameEn": "Taiwan",
    "flag": "🇹🇼"
  },
  "TT": {
    "code": "TT",
    "nameRu": "Trinidad and Tobago",
    "nameEn": "Trinidad and Tobago",
    "flag": "🇹🇹"
  },
  "TR": {
    "code": "TR",
    "nameRu": "Турция",
    "nameEn": "Turkey",
    "flag": "🇹🇷"
  },
  "LK": {
    "code": "LK",
    "nameRu": "Шри-Ланка",
    "nameEn": "Sri Lanka",
    "flag": "🇱🇰"
  },
  "TN": {
    "code": "TN",
    "nameRu": "Тунис",
    "nameEn": "Tunisia",
    "flag": "🇹🇳"
  },
  "TL": {
    "code": "TL",
    "nameRu": "Timor-Leste",
    "nameEn": "Timor-Leste",
    "flag": "🇹🇱"
  },
  "TM": {
    "code": "TM",
    "nameRu": "Туркменистан",
    "nameEn": "Turkmenistan",
    "flag": "🇹🇲"
  },
  "TJ": {
    "code": "TJ",
    "nameRu": "Таджикистан",
    "nameEn": "Tajikistan",
    "flag": "🇹🇯"
  },
  "LS": {
    "code": "LS",
    "nameRu": "Lesotho",
    "nameEn": "Lesotho",
    "flag": "🇱🇸"
  },
  "TH": {
    "code": "TH",
    "nameRu": "Таиланд",
    "nameEn": "Thailand",
    "flag": "🇹🇭"
  },
  "TF": {
    "code": "TF",
    "nameRu": "Fr. S. Antarctic Lands",
    "nameEn": "Fr. S. Antarctic Lands",
    "flag": "🇹🇫"
  },
  "TG": {
    "code": "TG",
    "nameRu": "Togo",
    "nameEn": "Togo",
    "flag": "🇹🇬"
  },
  "TD": {
    "code": "TD",
    "nameRu": "Chad",
    "nameEn": "Chad",
    "flag": "🇹🇩"
  },
  "LY": {
    "code": "LY",
    "nameRu": "Ливия",
    "nameEn": "Libya",
    "flag": "🇱🇾"
  },
  "AE": {
    "code": "AE",
    "nameRu": "ОАЭ",
    "nameEn": "United Arab Emirates",
    "flag": "🇦🇪"
  },
  "VE": {
    "code": "VE",
    "nameRu": "Венесуэла",
    "nameEn": "Venezuela",
    "flag": "🇻🇪"
  },
  "AF": {
    "code": "AF",
    "nameRu": "Afghanistan",
    "nameEn": "Afghanistan",
    "flag": "🇦🇫"
  },
  "IQ": {
    "code": "IQ",
    "nameRu": "Ирак",
    "nameEn": "Iraq",
    "flag": "🇮🇶"
  },
  "IS": {
    "code": "IS",
    "nameRu": "Исландия",
    "nameEn": "Iceland",
    "flag": "🇮🇸"
  },
  "IR": {
    "code": "IR",
    "nameRu": "Иран",
    "nameEn": "Iran",
    "flag": "🇮🇷"
  },
  "AM": {
    "code": "AM",
    "nameRu": "Армения",
    "nameEn": "Armenia",
    "flag": "🇦🇲"
  },
  "IT": {
    "code": "IT",
    "nameRu": "Италия",
    "nameEn": "Italy",
    "flag": "🇮🇹"
  },
  "VN": {
    "code": "VN",
    "nameRu": "Вьетнам",
    "nameEn": "Vietnam",
    "flag": "🇻🇳"
  },
  "AR": {
    "code": "AR",
    "nameRu": "Аргентина",
    "nameEn": "Argentina",
    "flag": "🇦🇷"
  },
  "AU": {
    "code": "AU",
    "nameRu": "Австралия",
    "nameEn": "Australia",
    "flag": "🇦🇺"
  },
  "IL": {
    "code": "IL",
    "nameRu": "Израиль",
    "nameEn": "Israel",
    "flag": "🇮🇱"
  },
  "IN": {
    "code": "IN",
    "nameRu": "Индия",
    "nameEn": "India",
    "flag": "🇮🇳"
  },
  "TZ": {
    "code": "TZ",
    "nameRu": "Танзания",
    "nameEn": "Tanzania",
    "flag": "🇹🇿"
  },
  "AZ": {
    "code": "AZ",
    "nameRu": "Азербайджан",
    "nameEn": "Azerbaijan",
    "flag": "🇦🇿"
  },
  "IE": {
    "code": "IE",
    "nameRu": "Ирландия",
    "nameEn": "Ireland",
    "flag": "🇮🇪"
  },
  "ID": {
    "code": "ID",
    "nameRu": "Индонезия",
    "nameEn": "Indonesia",
    "flag": "🇮🇩"
  },
  "UA": {
    "code": "UA",
    "nameRu": "Украина",
    "nameEn": "Ukraine",
    "flag": "🇺🇦"
  },
  "QA": {
    "code": "QA",
    "nameRu": "Катар",
    "nameEn": "Qatar",
    "flag": "🇶🇦"
  },
  "MZ": {
    "code": "MZ",
    "nameRu": "Mozambique",
    "nameEn": "Mozambique",
    "flag": "🇲🇿"
  }
};

window.GEOGRAPHY_PRESETS = {
  "all": {
    "id": "all",
    "title": "Весь мир",
    "subtitle": "Все доступные страны мира",
    "countries": [
      "BD",
      "BE",
      "BF",
      "BG",
      "BA",
      "BN",
      "BO",
      "JP",
      "BI",
      "BJ",
      "BT",
      "JM",
      "BW",
      "BR",
      "BS",
      "BY",
      "BZ",
      "RU",
      "RW",
      "RS",
      "LT",
      "LU",
      "LR",
      "RO",
      "GW",
      "GT",
      "GR",
      "GQ",
      "GY",
      "GE",
      "GB",
      "GA",
      "GN",
      "GM",
      "GL",
      "KW",
      "GH",
      "OM",
      "_2",
      "_1",
      "_0",
      "JO",
      "HR",
      "HT",
      "HU",
      "HN",
      "PR",
      "PS",
      "PT",
      "PY",
      "PA",
      "PG",
      "PE",
      "PK",
      "PH",
      "PL",
      "ZM",
      "EH",
      "EE",
      "EG",
      "ZA",
      "EC",
      "AL",
      "AO",
      "KZ",
      "ET",
      "ZW",
      "ES",
      "ER",
      "ME",
      "MD",
      "MG",
      "MA",
      "UZ",
      "MM",
      "ML",
      "MN",
      "MK",
      "MW",
      "MR",
      "UG",
      "MY",
      "MX",
      "VU",
      "FR",
      "FI",
      "FJ",
      "FK",
      "NI",
      "NL",
      "NO",
      "NA",
      "NC",
      "NE",
      "NG",
      "NZ",
      "NP",
      "CI",
      "CH",
      "CO",
      "CN",
      "CM",
      "CL",
      "CA",
      "CG",
      "CF",
      "CD",
      "CZ",
      "CY",
      "CR",
      "CU",
      "SZ",
      "SY",
      "KG",
      "KE",
      "SS",
      "SR",
      "KH",
      "SV",
      "SK",
      "KR",
      "SI",
      "KP",
      "SO",
      "SN",
      "SL",
      "SB",
      "SA",
      "SE",
      "SD",
      "DO",
      "DJ",
      "DK",
      "DE",
      "YE",
      "AT",
      "DZ",
      "US",
      "LV",
      "UY",
      "LB",
      "LA",
      "TW",
      "TT",
      "TR",
      "LK",
      "TN",
      "TL",
      "TM",
      "TJ",
      "LS",
      "TH",
      "TF",
      "TG",
      "TD",
      "LY",
      "AE",
      "VE",
      "AF",
      "IQ",
      "IS",
      "IR",
      "AM",
      "IT",
      "VN",
      "AR",
      "AU",
      "IL",
      "IN",
      "TZ",
      "AZ",
      "IE",
      "ID",
      "UA",
      "QA",
      "MZ"
    ]
  },
  "europe": {
    "id": "europe",
    "title": "Европа",
    "subtitle": "Все страны Европы (ЕС, Великобритания, Швейцария, Норвегия, Балканы и др.)",
    "countries": [
      "AL", "AT", "BA", "BE", "BG", "BY", "CH", "CY", "CZ", "DE",
      "DK", "EE", "ES", "FI", "FR", "GB", "GR", "HR", "HU", "IE",
      "IS", "IT", "LT", "LU", "LV", "MD", "ME", "MK", "NL", "NO",
      "PL", "PT", "RO", "RS", "SE", "SI", "SK", "UA"
    ]
  },
  "eu": {
    "id": "eu",
    "title": "Евросоюз (ЕС)",
    "subtitle": "27 государств-членов ЕС",
    "countries": [
      "AT",
      "BE",
      "BG",
      "HR",
      "CY",
      "CZ",
      "DK",
      "EE",
      "FI",
      "FR",
      "DE",
      "GR",
      "HU",
      "IE",
      "IT",
      "LV",
      "LT",
      "LU",
      "NL",
      "PL",
      "PT",
      "RO",
      "SK",
      "SI",
      "ES",
      "SE"
    ]
  },
  "middle_east": {
    "id": "middle_east",
    "title": "Ближний Восток",
    "subtitle": "ОАЭ, Саудовская Аравия, Катар, Турция, Оман и др.",
    "countries": [
      "AE",
      "SA",
      "QA",
      "KW",
      "OM",
      "TR",
      "IL",
      "JO",
      "LB",
      "IQ",
      "IR",
      "EG"
    ]
  },
  "cis": {
    "id": "cis",
    "title": "СНГ и Центральная Азия",
    "subtitle": "РФ, Беларусь, Казахстан, Узбекистан, Армения и др.",
    "countries": [
      "RU",
      "BY",
      "KZ",
      "UZ",
      "KG",
      "TJ",
      "TM",
      "AM",
      "AZ",
      "GE"
    ]
  },
  "asia": {
    "id": "asia",
    "title": "Азия и ЮВА",
    "subtitle": "Китай, Япония, Южная Корея, Таиланд, Сингапур и др.",
    "countries": [
      "CN",
      "JP",
      "KR",
      "IN",
      "TH",
      "MY",
      "ID",
      "VN",
      "PH",
      "LK",
      "TW"
    ]
  },
  "north_america": {
    "id": "north_america",
    "title": "Северная Америка",
    "subtitle": "США, Канада, Мексика",
    "countries": [
      "US",
      "CA",
      "MX"
    ]
  }
};
