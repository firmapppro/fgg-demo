...
1. **Блок «2. Основная информация о самолете»:**
   ...
   - Поле **«Валюта»** (`currencyId`): 
     - **Автоматическая смена валюты:**
       - Если в географии полетов выбраны только **РФ и/или Беларусь (`RU`, `BY`)**, валюта автоматически устанавливается в **«Российский рубль (RUB)»**.
       - При добавлении любых **европейских или международных стран** (пресет «Евросоюз», ОАЭ, Турция и т.д.) валюта автоматически переключается на **«Евро (EUR)»**.
     - **Ручной оверрайд администратором:** Администратор может в любой момент вручную изменить валюту в выпадающем списке (на USD, AED или принудительно RUB/EUR). При ручном изменении автоматическое переключение уступает место выбору администратора.
